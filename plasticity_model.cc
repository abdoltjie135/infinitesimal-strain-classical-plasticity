...
// Not all the following includes are necessary for this file
...
namespace PlasticityModel
{
    ...
    namespace EquationData
    {
        ...
        {
            if (component == 2 && p[2] == 1) // if z and z = 1
                return 0.1;
            else
                return 0.;
        }
    ...
    template <int dim>
    void PlasticityProblem<dim>::assemble_mass_matrix_diagonal(
        ...
        const TrilinosWrappers::MPI::Vector& linearization_point)
    {
        ...
        std::cout << "local_dof_indices before: ";
        ...
        // the following is for debugging purposes
        std::cout << "Number of active cells: " << dof_handler.get_triangulation().n_active_cells() << std::endl;
            ...
            if (cell->is_locally_owned())
            {
                ...
                for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
                {
                    ...
                    for (unsigned int i = 0; i < dofs_per_cell; ++i)
                    {
                        ...
                        {
                            /*
                            // Log the indices and the value being added
                            std::cout << "Adding to newton_matrix at position ("
                                << local_dof_indices[i] << ", " << local_dof_indices[j] << "): "
                                << stress_phi_i * fe_values[displacement].symmetric_gradient(j, q_point) * fe_values.
                                JxW(q_point)
                                << std::endl;
                            */
                ...
                cell->get_dof_indices(local_dof_indices);
                // the following is for debugging purposes
                std::cout << "local_dof_indices after: ";
                for (const auto& dof_index : local_dof_indices)
                {
                    std::cout << dof_index << " ";
                }
                ...
                std::cout << std::endl;
                if (local_dof_indices.size() != cell_matrix.m() || local_dof_indices.size() != cell_rhs.size()) {
                    std::cerr << "Error: Size mismatch between local_dof_indices and cell_matrix/cell_rhs." << std::endl;
                }
                for (const auto &index : local_dof_indices) {
                    if (index >= newton_matrix.m() || index >= newton_matrix.n()) {
                        std::cerr << "Error: Index " << index << " is out of bounds for newton_matrix." << std::endl;
                    }
                    if (index >= newton_rhs.size()) {
                        std::cerr << "Error: Index " << index << " is out of bounds for newton_rhs." << std::endl;
                    }
                }
                for (unsigned int i = 0; i < cell_matrix.m(); ++i) {
                    for (unsigned int j = 0; j < cell_matrix.n(); ++j) {
                        if (std::isnan(cell_matrix(i, j)) || std::isinf(cell_matrix(i, j))) {
                            std::cerr << "Error: NaN or infinite value found in cell_matrix at (" << i << ", " << j << ")." << std::endl;
                        }
                    }
                }
                for (unsigned int i = 0; i < cell_rhs.size(); ++i) {
                    if (std::isnan(cell_rhs(i)) || std::isinf(cell_rhs(i))) {
                        std::cerr << "Error: NaN or infinite value found in cell_rhs at index " << i << "." << std::endl;
                    }
                }

                // Validate local_dof_indices against newton_matrix dimensions
                std::cout << "Validating indices before distribution..." << std::endl;
                for (unsigned int i = 0; i < local_dof_indices.size(); ++i) {
                    if (local_dof_indices[i] >= newton_matrix.m()) {
                        std::cerr << "Error: local_dof_indices[" << i << "] = " << local_dof_indices[i]
                                  << " is out of bounds for newton_matrix." << std::endl;
                    }
                }

                // Check for NaN or Inf in cell_matrix
                for (unsigned int i = 0; i < cell_matrix.m(); ++i) {
                    for (unsigned int j = 0; j < cell_matrix.n(); ++j) {
                        if (std::isnan(cell_matrix(i, j)) || std::isinf(cell_matrix(i, j))) {
                            std::cerr << "Error: NaN or infinite value found in cell_matrix at (" << i << ", " << j << ")." << std::endl;
